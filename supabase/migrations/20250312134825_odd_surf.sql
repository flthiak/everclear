...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'production' AND policyname = 'Enable read access for authenticated users'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE tablename = 'godown_transfers' AND policyname = 'Enable read access for authenticated users'
  ) THEN
